...
var _ = Describe("Multus basic operations", func() {
	...
	Context("Simple macvlan delegate", func() {
		...
		BeforeEach(func() {
			var err error
			nad, err = clientset.nadClientSet.K8sCniCncfIoV1().NetworkAttachmentDefinitions(namespace).Create(context.TODO(), newMacvlanNetworkAttachmentDefinitionSpec(namespace, networkName, lowerDevice), metav1.CreateOptions{})
		...
		BeforeEach(func() {
			...
			for _, pod := range []*v1.Pod{firstPod, secondPod} {
				_, err = clientset.k8sClientSet.CoreV1().Pods(namespace).Create(context.TODO(), pod, metav1.CreateOptions{})
				Expect(err).NotTo(HaveOccurred(), "should have been able to create a pod")
			}
			for _, pod := range []*v1.Pod{firstPod, secondPod} {
				Eventually(func() bool {
					pod, err := clientset.k8sClientSet.CoreV1().Pods(namespace).Get(context.TODO(), pod.GetName(), metav1.GetOptions{})
		...
		AfterEach(func() {
			for _, pod := range []*v1.Pod{firstPod, secondPod} {
				Expect(clientset.k8sClientSet.CoreV1().Pods(namespace).Delete(context.TODO(), pod.GetName(), metav1.DeleteOptions{})).To(Succeed(), "should have been able to delete the pod")
			}
			for _, pod := range []*v1.Pod{firstPod, secondPod} {
				Eventually(func() bool {
		...
		AfterEach(func() {
			Expect(clientset.nadClientSet.K8sCniCncfIoV1().NetworkAttachmentDefinitions(namespace).Delete(context.TODO(), nad.GetName(), metav1.DeleteOptions{})).NotTo(HaveOccurred(), "should have been able to delete the NET-ATTACH-DEF")
		})
